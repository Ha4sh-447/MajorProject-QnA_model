...
* **Attempt**: Disabled all quantization and mixed precision.
* **Result**: Training was perfectly stable. Stable gradients allowed us to increase the **Learning Rate (5e-5)** and **LoRA Rank (r=32)**, giving the model enough capacity to learn all 6 tags.
...
To scientifically prove the model's improvement, we implemented 6 metrics. Here is the final performance comparison between the two models:

| Metric | Option A (6-Level) | Option B (4-Level) | Interpretation |
| :--- | :--- | :--- | :--- |
| **BLEU-1** | **0.2026** | 0.1936 | Higher in Option A due to ~50% more training data. |
| **ROUGE-L** | **0.2301** | 0.2296 | Comparable structural generation quality. |
| **BERTScore-F1** | **0.8646** | 0.8631 | Extremely strong semantic alignment with source text. |
| **Tag Accuracy** | **1.0000** (100%) | **1.0000** (100%) | **Perfect.** Both strategies completely fixed tag hallucination. |
| **Level Distinctness** | **0.6664** | 0.6514 | Option A questions are slightly more distinct from one another. |
| **Self-BLEU (↓ is better)** | 0.5276 | **0.4726** | Option B is more diverse and less prone to repetitive phrasing. |
...
4. **Tag Accuracy**: The percentage of outputs containing the correct `[TAG]`. The leap from ~60% to 100% was our primary objective.
5. **Level Distinctness**: A measure of how different a "Remember" question looks from an "Evaluate" question.
6. **Self-BLEU**: Measures internal diversity within generated questions. The **4-Level model (0.47)** produced more varied, natural-sounding questions than the **augmented 6-Level model (0.52)**, which relied on rigid templates for augmentation.
